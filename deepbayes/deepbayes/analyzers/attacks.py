...
def gradient_expectation(model, inp, direction, loss_fn, num_models=1):
    ...
    for i in range(num_models):
        if(model.det or val == 1 or val == -1):
            no_op = 0
        else:
...
# FGSM
def FGSM(model, inp, loss_fn, eps, direction=-1, num_models=1, order=1, samples=1):
    ...
    if(order == 1):
        ...
        grad = gradient_expectation(model, inp, direction, loss_fn, num_models)
    elif(order == 0):
